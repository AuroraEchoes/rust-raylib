    ...
    Self: Sized,
{
    ...
    fn draw_texture(&mut self, tex: &Texture, position: Vector2, params: DrawTextureParams) {
        // If a subsection is being drawn, scale relative to that only. Otherwise, scale relative
        // to the entire texture
        let (scl_x, scl_y) = match params.source {
            Some(source) => (source.width, source.height),
            None => (tex.width() as f32, tex.height() as f32),
        };
        unsafe {
            ffi::DrawTexturePro(
                tex.raw.clone(),
                ...
                Rectangle::new(
                    position.x,
                    position.y,
                    params.scale.x * scl_x,
                    params.scale.y * scl_y,
                )
                .into(),
                params.origin.into(),
                params.rotation,
